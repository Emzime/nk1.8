...
if(defined('TESTLANGUE')) { 
    ...
    function affichBlockCounter($blok){
        global $nuked;

        $visites = 0;
        $dbsCount = 'SELECT count 
                     FROM '.STATS_TABLE.' 
                     WHERE type = "pages"';
        $dbeCount = mysql_query($dbsCount);
        while (list($count) = mysql_fetch_array($dbeCount))
        {
        ...
        $blok['content'] = '<div class="nkAlignCenter">'.$visites.'</div>';
