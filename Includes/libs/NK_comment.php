    ...
    **/
    ...
    function viewComment($module, $idItem, $nbComment = null, $splitComment = null, $entente= null) {
        ...
        if($active == 1  && $visiteur >= $level_access && $level_access > -1) {
            ...
            ?>
            <div class="nkMarginTop15 nkMarginBottom15">
                ...
                while(list($titre, $comment, $autor, $autor_id, $date) = mysql_fetch_array($sqlComment)) {                   
                    ...
                    if(!is_null($splitComment)){$comment =  $GLOBALS['nkFunctions']->nkCutText($comment, $splitComment);}
                        
                    ?>
                    <div class="nkBoxcontainer">
                        <?php echo $comment; ?>
                        <p><?php echo COMMENTBY; ?>:&nbsp;<?php echo $autor; ?>&nbsp;<?php echo COMMENTTHE; ?>&nbsp;<small><?php echo nkDate($date); ?></small></p>
                    </div>
                <?php
                }
                ...
                if($count >= 1) {
                    ...
                    if(is_null($nbComment)){
                        $link = '<a class="nkPopupBox nkButton nkButton'.$module.'" href="index.php?file='.$module.'&amp;nuked_nude=index&amp;op=post_com&amp;idItem='.$idItem.'&amp;module='.$module.'">'.ADDCOMMENT.'</a>';
                    }else{
                        $link = '<a class="nkPopupBox nkButton nkButton'.$module.'" href="index.php?file='.$module.'&amp;nuked_nude=index&amp;op=viewComment&amp;idItem='.$idItem.'&amp;module='.$module.'">'.SEEALLCOMMENT.'</a>';
                    }
                }else{
                    $link = '<a class="nkPopupBox nkButton nkButton'.$module.'" href="index.php?file='.$module.'&amp;nuked_nude=index&amp;op=post_com&amp;idItem='.$idItem.'&amp;module='.$module.'">'.NEWCOMMENT.'</a>';                
                    ?>
                    <p class="nkAlignCenter"><?php echo NOCOMMENTDB; ?></p>
                <?php
                }
                ?>
            </div>
    ...
    function post_comment($module, $idItem, $title, $text, $nick){
        ...
        ?>
            <label id="nkCenter"><?php echo BADCODECONFIRM; ?></label>
        <?php
        }else{
            ...
            if($active == 1  && $visiteur >= $level_access && $level_access > -1) {
                ...
                if ($nick == "error1"){
                ?>
                    <p id="nkCenter"><?php echo NONICK; ?></p>
                <?php
                }else if ($nick == "error2"){
                ?>
                   <p id="nkCenter"><?php echo RESERVNICK; ?></p>
                <?php
                }
                else if ($nick == "error3"){
                ?>
                    <p id="nkCenter"><?php echo BANNEDNICK; ?></p>
                <?php
                }else if($title == ""){
                ?>
                    <p id="nkCenter"><?php echo NOTITLE; ?></p>
                <?php
                }else if($text == ""){
                ?>
                    <p id="nkCenter"><?php echo NOTEXT; ?></p>
                <?php
                }else{
                    ...
                    if($module != "" && $idItem != ""){
                        ...
                        ?>
                            <p id="nkCenter"><?php echo NOFLOOD; ?></p>
                            <?php
                            ...
                            <p id="nkCenter"><?php echo COMMENTADD; ?></p>
                        <?php
                        }
                    }else{
                    ?>
                        <p id="nkCenter"><?php echo COMMENTNOTADD; ?></p>
                    <?php
                    }
                }
            }
    ...
    function del_comment($cid){
        ...
        if ($visiteur >= $level_admin){
            ...
            ?>
            <p id="nkCenter"><?php echo COMMENTDEL; ?></p>
            <?php
            redirect('index.php?file='.$module.'&nuked_nude=index&op=viewComment&idItem='.$idItem.'&module='.$module, 2);
        }else{
            echo $GLOBALS['nkFunctions']->nkBadLevel();
    ...
    function modif_comment($cid, $titre, $texte, $module, $idItem){
        ...
        if ($visiteur >= $level_admin){
            ...
            $upd = mysql_query($upd);
            ?>
            <p id="nkCenter"><?php echo COMMENTMODIF; ?></p>
